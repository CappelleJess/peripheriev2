    ...
    <table className="w-full table-auto border-collapse text-white">
      <thead>
        <tr className="bg-[#6b728e]">
          ...
          <th className="p-2 border">Choix</th>
      ...
      <tbody>
        {assets.map((asset) => (
          <tr key={asset._id} className="hover:bg-[#2e2e2e]">
            ...
            <td className="p-2 border">
              <button onClick={() => onEdit(asset)} className="text-blue-300 hover:text-blue-500 mr-2">✏️</button>
              <button onClick={() => onDelete(asset._id)} className="text-red-400 hover:text-red-600">🗑️</button>
            </td>
          </tr>
        ))}
      </tbody>
